...
const Project = ({ project }: { project: TProject }) => {
  return (
    <div
      className="group/container relative mx-auto flex h-[28rem] w-full flex-col gap-0 overflow-hidden rounded-lg border-2 border-neutral-800 bg-neutral-900 text-center md:h-96 md:max-w-3xl md:flex-row md:gap-10 md:text-start md:even:flex-row-reverse md:[&>*]:basis-1/2"
      data-pattern="stripes"
      style={{ '--opacity': '0.03' } as React.CSSProperties}
    >
      {project.isDemo && (
        <span className="absolute left-1/2 top-0 -translate-x-1/2 rounded-b-lg border-x-2 border-b-2 border-neutral-800 bg-red-800 px-2 py-1 text-xs font-semibold text-white md:group-odd/container:left-0 md:group-odd/container:translate-x-0 md:group-odd/container:rounded-bl-none md:group-odd/container:rounded-br-lg md:group-odd/container:border-l-0 md:group-odd/container:border-r-2 md:group-even/container:left-full md:group-even/container:-translate-x-full md:group-even/container:rounded-bl-lg md:group-even/container:rounded-br-none md:group-even/container:border-l-2 md:group-even/container:border-r-0">
          Demo
        </span>
      )}
      <div className="flex flex-col px-4 py-7 md:pb-7 md:pl-10 md:pt-10 md:group-even/container:pl-4 md:group-even/container:pr-10">
        ...
        <p className="mt-3 line-clamp-6 text-base">{project.description}</p>
        <div className="mt-auto flex flex-col gap-3">
          ...
          <Link
            ...
            className="w-full rounded-lg border border-neutral-800 bg-neutral-700 py-2 text-center transition-colors duration-200 hover:bg-neutral-700/80"
          >
